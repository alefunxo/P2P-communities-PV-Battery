...
class SCCommunity(Community):
    ...
    def getSelfConsumptionCommunity(self):
        '''Diferentiated bill individual PV and battery (equivalent to shared battery), without export from the battery'''
        self.nested_out={}
        j=0
        k=0
        self.sum_bill=0
...
class SCCommunityClassic(Community):
    
    def getSelfConsumptionCommunity(self):
        '''Unique bill divided among the users and shared PV and battery'''
        self.nested_out={}
        j=0
        k=0
        self.sum_bill=0
            ...
            if self.PtC!=False:
                ...
                if self.selection.loc[i,'sub_'+str(self.pv_penetration)+'_100']:#all with PV
                    ...
                    if self.selection.loc[i,'sub_'+str(self.pv_penetration)+'_'+
                                          str(self.batt_penetration)]: #if battery
                        self.Batt_size_comm+=10
        self.param_tech_comm.update({'BatteryCapacity':self.Batt_size_comm,
                                     'BatteryEfficiency': 0.91,
                                     'InverterEfficiency': 0.94,
                                     'MaxPower': 100})
        self.nested_out=p2p.dispatch_max_sc(self.df_out.PV*self.PV_size_comm, self.df_out.loc[:,list(self.df_com.iloc[:,0])].sum(axis=1),self.PV_size_comm,self.param_tech_comm)                   
                    #self.df_out.loc[:,list(self.df_com.iloc[:,0])]#select all demand data and add it 
        self.out=p2p.print_analysis_prices(self.df_out.PV*self.PV_size_comm, 
                 self.df_out.loc[:,list(self.df_com.iloc[:,0])].sum(axis=1),self.retail_price_sc,
                 self.export_price_sc, self.param_tech_comm,self.nested_out,
                 isCommunity=True)
    def runAll(self):
        self.createParams()
        self.selectData()
        self.getSelfConsumptionCommunity()
        self.out['bill']=((self.nested_out['grid2load'] * self.timestep) * self.retail_price_sc - (self.nested_out['inv2grid'] * self.timestep ) * self.export_price_sc).sum()
        self.out['cut']=self.cut
        self.out['Comm']=self.Comm
class P2PCommunity(Community):
    def getPrices(self,SCCommunity):
        ...
        #self.df_prices=p2p.price_generation2(SCCommunity.out_comm_final, self.param_tech_no_batt)
                ...
                if self.selection.loc[i,'PV_size']>0:#all with PV
                    ...
                    if self.selection.loc[i,'battery']: #if battery
                        ...
                        self.result_out[i]['bill']=p2p.bill_hh_p2p(self.df_out.PV*self.selection.PV_size[i],
                                                 ...
                                                 self.nested_out[i],self.param_tech)#,batt_revenue=True)
                        self.result_out[i]['type']='PV_batt'
                        j+=1
                    else: #if only PV battery=0 kWh, thus no bhv needed
                        self.result_out[i]=p2p.print_analysis_prices(self.df_out.PV*self.selection.PV_size[i],
